...
int WebRtcNs_InitCore(NoiseSuppressionC* self, uint32_t fs) {
  ...
  memset(self->analyzeBuf, 0, sizeof(float) * ANAL_BLOCKL_MAX);//滑动分析窗，fft
  memset(self->dataBuf, 0, sizeof(float) * ANAL_BLOCKL_MAX);//储存原始的时域信号
  memset(self->syntBuf, 0, sizeof(float) * ANAL_BLOCKL_MAX);//谱减法，减去噪声后变换到时域的信号

  // For HB processing.高频信号部分，最多有两个band(2*256)。
  memset(self->dataBufHB,
         0,
         sizeof(float) * NUM_HIGH_BANDS_MAX * ANAL_BLOCKL_MAX);

  // For quantile noise estimation.分位数噪声估计
  ...
  // Initialize variables for new method.初始化，设置噪声估计所用到的信号
